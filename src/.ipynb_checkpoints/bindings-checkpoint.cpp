...
PYBIND11_MODULE(pynocchio, m)
{
    ...
    py::class_<Quaternion<double>>(m, "Quaternion")
        .def(py::init<>())
        .def(py::init<const Quaternion<double> &>())
        .def(py::init<const Vector<double, 3> &, const double &>())
        .def("inverse", &Quaternion<double>::inverse)
        .def("getAngle", &Quaternion<double>::getAngle)
        .def("getAxis", &Quaternion<double>::getAxis);
    ...
    py::class_<Transform<double> >(m, "Transform")
        ...
        .def(py::init<const Quaternion<double> &, double, const Vector3 &>(),
            py::arg("inRot"),
            py::arg("inScale"),
            py::arg("inTrans"))
